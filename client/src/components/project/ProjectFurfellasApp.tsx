...
const ProjectFurfellasApp:FC = () => {
  ...
  return (
    <Box>
      <ProjectHeader 
        href={process.env.REACT_APP_PROJECT_FURFELLAS_LINK} 
        title={translate("furfellasAppName")} 
      />
      <Box 
        sx={{ 
          display: 'flex', 
      ...
      >
        ...
        <Box sx={{ padding: {mobile:1, tablet: 2}}}>
            <ProjectFeatureBox name={"furfellasApp"} title={"Front"} features={[
              translate("furfellasAppFrontFeatureOne"),
              translate("furfellasAppFrontFeatureTwo"),
              ]}
            />
            <Link href={process.env.REACT_APP_PROJECT_FURFELLAS_APP_GITHUB_FRONT} >
              <GitHubIcon />
            </Link>
            <ProjectFeatureBox name={"furfellasApp"} title={"Back"} features={[
