            ...
            {data &&
                data.map(item => (
                    <TimelineItem key={item.title.KR}>
                        <TimelineOppositeContent sx={{ m: 'auto 0', padding: '8px 8px' }} align="right" variant="body2" color="text.secondary">
                            {item.date ? `${convertDateToStr(item.date.seconds)}` : `${convertDateToStr(item.startDate.seconds)} ~ ${convertDateToStr(item.endDate.seconds)}`}
