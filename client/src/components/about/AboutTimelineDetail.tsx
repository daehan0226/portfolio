    ...
    return (
        <StyledEngineProvider injectFirst>
            <Accordion sx={{backgroundColor:"primary.100", boxShadow: "none"}} >
                ...
                >
                <Typography variant='h6' sx={{wordBreak: "break-word"}}>{title[language]}</Typography>
                </AccordionSummary>
                <AccordionDetails>
                    {detail && detail.map(({project, tasks})=>(
                        <div key={project[language]}>
                            <Typography variant='body1' sx={{paddingLeft:1}}>{project[language]}</Typography>
                            {tasks.map((task,i)=>(
                                <Typography key={`${project}-${i}`} variant='body2' sx={{paddingLeft:2}}>{task[language]}</Typography>
                            ))}
                        </div>
                    ))}
                </AccordionDetails>
